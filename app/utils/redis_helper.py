...
class TokenHelper:
    ...
    @staticmethod
    def get_all_tokens(user_id: str, token_type: Literal["access_token", "refresh_token", "impersonation_token"]):
    ...
    @staticmethod
    def delete_all_tokens(user_id: str, token_type: Literal["access_token", "refresh_token", "impersonation_token"]):
        pattern = f"{token_type}:{user_id}:*"
        keys = redis_client.keys(pattern)
        if keys:
    ...
    @staticmethod
    def blacklist(token: str, expires_delta: int):
...
class SimpleHelper:
    ...
    # value 관련 메서드
    def store_value(self, identifier: str, time_seconds: int, value: str):
    ...
    def delete_value(self, identifier: str):
    ...
    def get_ttl(self, identifier: str):
        return redis_client.ttl(f"{self.prefix}:{identifier}")

    # count 관련 메서드
    def increment_count(self, identifier: str):
...
class BaseVerificationHelper:
    ...
    def get_send_count(self, identifier: str):
        count = redis_client.get(f"{self.prefix}_send_count:{identifier}")
        return int(count) if count else 0

    def get_send_count_ttl(self, identifier: str):
        """send_count 키의 남은 시간을 반환합니다."""
        return redis_client.ttl(f"{self.prefix}_send_count:{identifier}")

    def set_verification_complete(self, identifier: str, device_id: str):
        """인증 완료 정보 저장 (10분 만료)"""
        key = f"{self.prefix}_verified:{identifier}:{device_id}"
        redis_client.setex(key, 600, "1")  # 10분 만료, 단순히 "1" 값만 저장

    def get_verification_complete(self, identifier: str, device_id: str):
        """인증 완료 정보 조회"""
        key = f"{self.prefix}_verified:{identifier}:{device_id}"
        return redis_client.exists(key)

    def del_verification_complete(self, identifier: str, device_id: str):
        """인증 완료 정보 삭제"""
        key = f"{self.prefix}_verified:{identifier}:{device_id}"
        redis_client.delete(key)

    def is_verified(self, identifier: str, device_id: str):
        """인증 완료 여부 확인"""
...
class RateLimiter:
    ...
    def get_remaining_requests(self, key: str):
        redis_key = f"{self.prefix}:{key}"
        current_count = redis_client.get(redis_key)
        if current_count is None:
            return 0
    ...
    def get_reset_time(self, key: str):
        redis_key = f"{self.prefix}:{key}"
